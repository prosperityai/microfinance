...
CREATE TABLE logs.lg_customers (
	lg_customer_id			serial primary key,
	customer_id				integer,
	entity_id 				integer,
...
CREATE OR REPLACE FUNCTION log_customers() RETURNS trigger AS $$
BEGIN
...
CREATE TRIGGER log_customers AFTER UPDATE OR DELETE ON customers
	FOR EACH ROW EXECUTE PROCEDURE log_customers();
